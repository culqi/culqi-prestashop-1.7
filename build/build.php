...
$version = get_module_version("$root/config_es.xml");
$dist = "$root/dist";
$temp = "$dist/tmp/culqi";
$zipOut = "$dist/culqi-prestashop-$version.zip";
$excludes = array_filter(
    array_map('trim', file("$root/build/exclude.txt")),
    fn($l) => $l !== '' && !str_starts_with($l, '#')
);

echo "=== Culqi Prestashop Plugin Build ===\n\n";
...
echo "\n✅ Build generado: dist/culqi-prestashop-$version.zip\n";
...
function get_module_version(string $configPath): string
{
    ...
    if (!$xml || !isset($xml->version)) {
        throw new Exception('No se pudo leer la versión del módulo desde config_es.xml');
    }
