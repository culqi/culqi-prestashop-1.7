...
class Culqi extends PaymentModule
{
    ...
    public function getCulqiInfoCheckout()
    {
        ...
        $data = array(
            ...
            "url_charge_ajax" => $this->context->link->getModuleLink('culqi', 'chargeajax', [], true),
            "url_postpayment"     => $this->context->link->getModuleLink('culqi', 'postpayment', [], true)
        );

        Media::addJsDef(array('data_culqi' => $data));
        return $data;
    }
    ...
    {
        if (!parent::uninstall() || !Configuration::deleteByName('CULQI_STATE_OK') || !Configuration::deleteByName('CULQI_STATE_ERROR') || !Configuration::deleteByName('CULQI_LLAVE_SECRETA') || !Configuration::deleteByName('CULQI_LLAVE_PUBLICA') || !$this->uninstallStates()) return false;
        return true;
    }
    ...
    {
        $this->context->controller->addCSS($this->_path.'views/css/culqi.css');
        $this->context->controller->addCSS($this->_path.'views/css/waitMe.min.css');

        $this->getCulqiInfoCheckout();

        $this->context->controller->registerJavascript(
            'jquery', 'https://ajax.googleapis.com/ajax/libs/jquery/2.1.4/jquery.min.js', array(
            'server'   => 'remote',
            'position' => 'head',
            'priority' => 20
            )
        ...
        $this->context->controller->registerJavascript(
            'culqi', 'https://checkout.culqi.com/v2', array(
            'server'   => 'remote',
            'position' => 'head',
            'priority' => 21
            )
        ...
        $this->context->controller->addJS($this->_path.'views/js/front/culqi.js');

        $this->context->controller->registerJavascript(
            'jqueryculqi', 'https://www.culqi.com/libs/jquery.culqi.js', array(
            'server'   => 'remote',
            'position' => 'head',
            'priority' => 22
            )
